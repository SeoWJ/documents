...
## Chapter 2. 멀티 테넌트 클라우드에서 애플리케이션 프로젝트 관리
 

### 프로젝트 환경 생성
- ID 서비스 소개
    - 모든 오픈스택 활동의 사용자 및 서비스 인증과 권한을 처리
    - 리전, 도메인, 프로젝트 및 하위 프로젝트 별 리소스 관리.(리전 - 도메인 - 프로젝트 - 하위 프로젝트 순으로 큰 단위라 보면 된다.)
    - 프로젝트는 리소스 격리용으로 사용한다. (CPU는 얼마나, 메모리는 얼마나 쓸 수 있다를 제한걸기 위해서 사용. 프로젝트별로 사용가능 할당량을 제공)
    - 도메인 운영자는 도메인과 프로젝트를 사용하여 리소스 관리/제한. 리전은 권한이 없다. 기본 도메인은 default로 설정되어 있다.. (* 도메인은 keystone이 3 버전 이상일 때만 사용 가능)
    - Default 도메인, admin 프로젝트, admin 사용자 등이 기본 제공.
    - 일반적인 ID관리 시나리오에서는 조직의 계정으로 외부 LDAP(외부 디렉토리 서비스) 프로바이더 사용.
    - 엔드포인트(접근 가능한 서비스 API)를 통해 인증 후 권한 부여

- Openstack 도메인 및 프로젝트 관리.
    - 프로젝트는 중첩 및 역할 상속이 가능.
    - 프로젝트는 리소스의 집합이며 도메인에 소속.
    - 사용자는 여러 프로젝트에 역할 할당 및 전환이 가능
    - 프로젝트 삭제 시 사용자 할당과 일부 리소스만 제거.(여러 프로젝트에 접근 가능한 사용자가 있을 수 있으니 프로젝트 삭제시에 사용자를 같이 지워버릴 수는 없다. 리소스 역시 프로젝트에 할당하는 방식이기는 하나 우리가 만드는 인스턴스, 네트워크, 이미지 등이 프로젝트 안에서만 쓸수있는 프라이빗이면 상관없으나 퍼블릭인 경우 같이 지워버릴수는 없다.)
    - 프로젝트를 만들고 삭제하고는 당연히 어드민 권한이 있는 사용자만 가능하다.

- 도메인 및 사용 사례
    - 프로젝트 사용자 및 리소스의 컬렉션. 상위개념이다.
    - 리소스를 격리하는 공간을 정의.
    - 클라우드 관리자만 생성 가능.
    - admin 프로젝트의 리소스는 모두 공유.
    - 개별 ID관리 백엔드를 사용하여 개별 도메인을 생성하는 서비스 계정에만 사용.
    - 인증 방법 백엔드를 기준으로 도메인을 분리.

- Commands
    - `openstack project create <project name>` : 프로젝트를 생성한다. 어드민 권한이 있어야만 사용 가능.
        - `--domain <domain>` : 도메인 설정 옵션. 미설정시 default 도메인에 생성.
        - `--parent <project>` : 서브프로젝트를 만들때 사용. 상위 프로젝트를 지정.
        - `--description <description>` : 설명 기재.
        - `property <key=value>` : 메타데이터 설정.
        - `cat admin-rc` or `cat <사용자명>-rc` 파일에서 사용자의 프로젝트를 확인할 수 있다. 이 파일에 사용가능한 프로젝트가 지정되어 있으니 잘 확인해서 사용해야 한다.
    - `openstack project delete <project name>` : 프로젝트 제거. 다른 도메인에 만들었다면 옵션을 붙여서 제거해주어야 한다. 역시 어드민 권한이 있어야 사용 가능.
    - `openstack project list` : 프로젝트 리스트 확인. 본인이 접근할 수 있는 프로젝트 목록만 확인 가능하다.

    - `openstack user create <user name>` : 유저 생성. 어드민 권한 필요.
        - `--domain <domain>` : 도메인 설정 옵션
        - `--password <pw>` : 패스워드 설정.
    - `openstack role add --user-domain <domain> --user <user> --project-domain <domain> --project <project> member` : 유저에 멤버권한 부여.

    - `openstack domain create <domain name>` : 도메인 생성. 당연히 어드민만 사용 가능
        - `--enable` or `--disable`
    - `openstack domain set --disable <domain name>` 이후, `openstack domain delete <domain name>` : 도메인 삭제. 디스에이블을 먼저 해주어야 한다.

- 대시보드 이용시
    - 최상단의 `identity` 클릭 시 사용 가능한 프로젝트 확인 가능.
 

## 프로젝트에 대한 액세스 권한 제공

- LDAP : 도메인 관리 + 사용자 통합 관리 + 인증관리를 담당해 주는 서비스. 인증부분은 빠져있을 수도 있다. 서비스마다 다름. 기본적으로 DNS서비스로 주소를 계층적으로 디렉토리 구조 형식으로 관리해 주는데, 사용자 계정을 도메인 별로 사용자 이름들을 따로 관리해주는 서비스. 원래는 시스템이 여러대 있으면 유저 애드 명령어로 따로 사용자를 만들어 주어야 한다. 이를 통합하여 관리하기 불편한데 LDAP을 이용하면 하나로 통합하여 관리할 수 있다. 중앙에서 계정관리를 통합하여 관리해주는 서비스를 말한다.

- Openstack 도메인 고려 사항
    - 도메인은 자율적 조직의 사용자 및 리소스를 구성하는데 사용
    - 물리적 리소스에 대한 실질적인 소유권 또는 관리 통제권에 따라 분할
    - 사용자는 외부 ID관리 서비스에서 관리
    - 외부 ID 백엔드 사용시 내부 ID서비스의 데이터베이스에 별도 구성할 필요 없음
    - 실제 사용시에는 인식 및 역할 할당을 위해 명령어 등으로 생성.

- Openstack 사용자 소개
    - 사용자를 만드는건 LDAP이 해줄 것.
    - 프로젝트마다 적절한 사용자 역할 할당 필요.
    - 사용자는 여러 도메인에 역할 할당 가능
    - 리소스는 여러 도메인에서 공유 불가.
    - 서비스 프로젝트의 서비스 사용자는 서비스간 인증에서 사용.
    - 사용자는 할당된 역할에 따라 제한된 권한
    - 인증은 사용자 ID를 확인하는 프로세스
    - 다양한 인증 방식 지원
    - 인증에 따라 인증 토큰 제공
    - API V3에서는 사용자들의 집합인 그룹 개념 제공.

- Openstack 역할 설명
    - 토큰 범위
        - 범위 없음 : 권한 정보 없이 사용자 인증만 가능
        - 프로젝트 : 단일 프로젝트 내의 리소스 접근 권한 정의
        - 도메인 : 특정 도메인에서 사용자/그룹/프로젝트 관리 기능 정의
        - 시스템 : 노드 및 해당 서비스 등 플랫폼 자체를 관리하는 기능 정의
    - 기본 역할
        - admin : 지정된 범위에서 전체 접근 제공
        - member : 오브젝트 생성 기능 제공
        - reader : 오브젝트 나열 가능.

- Commands
    - `openstack group create <group name>` : 그룹 생성.
    - `openstack role list` : 현재 만들어진 역할을 확인한다.
    - `openstack role add --user-domain <user domain name> --user <user name> --system all admin` : 전체 시스템 범위로 어드민 권한 부여.
    - `openstack role assignment list` : 역할을 설정해둔 값들을 보는 것.. 어떤 사용자가 어느 프로젝트에 역할을 가지고 있는지를 테이블 형태로 보여준다. 더럽게뜨니까 --fit-width나 --name 걸 것.
 

### 프로젝트의 리소스 제한 구성

- 프로젝트 할당량 소개
    - 도메인 운영자가 해당 도메인의 모든 프로젝트에 관한 제한을 관리
    - 프로젝트 할당량은 운영 체제 리소스 할당량과 유사
    - 프로젝트별로 설정되어 할당 가능 리소스 제한


- commands
    - `openstack quota list` : 프로젝트 할당량 관련 리스트 출력
        - `--compute`, `--volume`, `--network` 별로 확인 가능함.
    - `openstack quota show` : 컴퓨트, 네트워크등으로 분류하지 않고 전체 확인 가능.
    - injected-files : 인스턴스를 생성하면서 집어넣을 파일이란 의미.
    - `openstack quota set` : 할당량 설정.

- Server Group
    - 가급적 인스턴스들을 같은 노드위에서 실행시켰으면 좋겠다 싶을 때 설정하는 것.
    - 서버 그룹 역시 할당량이 존재한다.



## Chapter 3. Red Hat Openstack Platform 네트워크 관리


### TCP/IP 기본 사항 검토

- IPv4 네트워킹
    - 애플리케이션 계층 : 서버/클라이언트 간의 통신 사양 지정
    - 전송 계층 : TCP / UDP
    - 네트워크 계층 : IP주소를 이용해서 데이터 전달. 라우터로 여러 네트워크 연결 및 트래픽 라우팅
    - 링크 계층 : 물리적 미디어에 대한 연결 제공(이더넷, 무선 WLAN 등). 각 대상 식별을 위해 MAC 주소 사용.

- IPv4 주소
    - 32비트로 0~255 사이의 값을 4개 사용하여 표현.
    - 네트워크 파트와 호스트 파트로 구분
    - 네트워크 주소가 같으면 동일 네트워크로 직접통신을 하며 호스트 주소로 구분
    - 네트워크 주소 범위는 가변적(고정 클래스 + subnet) - 넷마스크로 구분.
    - 넷마스크 표현식 255.255.255.0 또는 /24로 표기.

- 이더넷 헤더(2계층 헤더)
    - MAC 주소 기본(보내는 곳과 받는 곳)
    - VLAN 태그 포함(옵션. VLAN설정이 되어 있을때만 포함된다.)

- 네트워크 장치
    - 세그먼트와 시스템 간에 패킷을 이동할 수 있는 네트워크 처리 기능을 제공.
    - 브릿지(스위치) : 2계층 장비로 각 세그먼트들을 연결하는 장치. 요즘은 거의 안쓰고 스위치를 쓴다. 가상머신에서는 브릿지를 많이 사용.
    - 라우터 : 3계층 장비로 IP주소로 라우팅 기능 제공
    - 허브 및 스위치 : 동일 네트워크에 있는 장치 연결. 데이터 전달 방식 상이.

- 브로드캐스트 도메인
    - 하나의 라우터 안에 있는 범위를 하나의 네트워크 대역이라 부른다.
    - 이 네트워크 대역에 있는 모든 엔드포인트 장비들을 하나의 브로드캐스트 도메인으로 본다.
    - VLAN을 따로 설정했다면 그부분은 따로 친다.
    - 대표적으로 처음 연결되거나 하는 경우 전체에다가 브로드캐스트를 때린다. 이런때 주로 사용. 성능저하를 유발한다. 따라서 브로드캐스트 도메인이 크면 네트워크의 성능이 안좋아질수있다.

- IP 및 이더넷 통합
    - 간단하게 말해서 DNS/IP/MAC의 3가지 주소 간에 서로 변환이 가능하여 사용이 가능하다는 의미이다.

- 가상 LAN(VLAN)으로 파티션
    - 네트워크를 논리적으로 파티셔닝해서 브로드캐스트 도메인 분리.
    - 확장성, 보안성, 관리 등에 유리하다.
    - 라우터는 브로드캐스트 필터링, 보안, 주소요약, 트래픽 플로우 관리 기능을 제공한다.
    - VLAN으로 나누어졌다면 같은 스위치 내애 있더라도 통신은 라우터를 통해야만 가능해진다.
    - VXLAN : 기존에는 4바이트 VLAN 헤더로 충분했지만 이제는 가상머신 올려대고 하면서 4바이트로 모자라졌다. 확장버전.


### 소프트웨어 정의 네트워크 소개

- 소프트웨어 정의 네트워크 처리
    - 물리적인 라우터와 소프트웨어 라우터는 유사. 라우터기능을 소프트웨어적으로 지원할 수 있도록 하는 중이다. 이를 SDN(Software Defined Network)이라 한다.
    - 플로우 테이블의 개념을 사용
        - 라우팅 테이블은 그냥 IP주소를 정해준다. 경로를 라우터가 계산해주기 때문에, 새 인터페이스가 추가되면 추가됐다고 라우터들끼리 서로 통신해서 업데이트를 하는 방식이었다. 따라서 라우터의 사양이 높아야 하는 문제가 있었다.
        - 소프트웨어 라우터는 메인 하나의 라우터(컨트롤플레인)가 모든 계산을 다 한다. 이 결과를 각각의 라우터 장비에 전달을 해주어 결과에 따라 패킷을 보내면 된다. 이를 플로우 테이블이라 한다.
    - 경로 및 MAC주소를 추적하는 대신 단일 조회 테이블을 사용
    - 패킷을 전달할지 아니면 삭제할지 결정
    - 플로우 테이블에는 통계등과 같은 네트워크기 기능이 포함.
    - TCP/IP 스트림의 첫번째 패킷은 소프트웨어에서 주로 처리
    - 스위치에서 생성된 플로우 규칙을 사용하여 패킷이 대상에 전달.

- OVN(Open Virtual Network) 소개
    - OVN은 2,3계층 네트워킹을 모두 제공
    - 보안그룹, DHCP, 라우팅 및 NAT 기능 포함
        - DHCP : 우리가 사용하는 각 네트워크 인터페이스들의 ip주소를 자동으로 할당해주는 서비스. 이게 없으면 하나하나 수동으로 설정해야 한다. 과거에는 DHCP를 dnsmasq라는 프로세스로 했었음.
        - NAT : network address translation. 네트워크 주소 변환. 사설주소를 공인주소로 변환해주는 것. 여기서 더 나아가 포트주소까지 변환하면 NAPT라 한다.
        - 보안 그룹 : 리눅스나 윈도우 등에서 사용하는 방화벽을 클라우드에서 사용하는 것. 여기에 맞게 패킷을 제어해주는 것을 OVN이 담당한다.
    - 오픈스택에서 OVN은 GENEVE 터널 오버레이 네트워크를 사용
        - 오버레이 네트워크 : 실제로는 스위치 라우터를 많이 거쳐야 하나 다이렉트로 연결된거처럼 사용할 수 있게 해주는 터널링 기술.
    - OVS ML2 드라이버의 제한사항 및 문제 해결
        - 2계층, 3계층 기능 따로 구성했었는데 그중 2계층을 구성했던게 ML2 드라이버.
    - Neutron 에이전트 사용 안함(확장성 향상). 서비스 자체를 안쓴다는건 아니고 에이전트를 안쓴다는 것.
    - 모든 호스트에서 ovn-controller 데몬 실행
    - 물리적 네트워크 없이 인스턴스와 컨테이너에 연결해서 성능 향상에 유리
    - Linux 브리지, dnsmasq 인스턴스 및 네임스페이스 필요 없음.

- OVN 아키텍처
    - P99 장표 참조.
    - 컨트롤러 노드에서 OVN ML2 플러그인을 사용하여 논리적 OVN 네트워킹 구성으로 변환.
    - OVN NB 데이터베이스에 ML2 플러그인에서 전달받은 논리적 네트워킹 구성 저장
    - OVN NorthBound 서비스로 논리적 경로 흐름으로 변환
    - OVN northd 데몬으로 OVN SouthBound 데이터베이스에 논리적 경로 흐름 저장(컨트롤 노드)(내가 통신을 할 때 어디로 전달해야 하는가를 저장한다는 의미)
    - ovn-controller는 SB-DB에 연결해 네트워크 트래픽 제어 및 모니터링
    - OVN 메타데이터 에이전트는 HAProxy 인스턴스를 생성
        - OVS 인터페이스, 네트워크 네임스페이스, HAProxy 프로세스를 관리

- OVN 데이터베이스
    - 인프라 규모, 지리적 위치, 트래픽 양, 성능 등에 요소에 따라 위치 선택
    - OVN 작동을 위해서는 하이퍼바이저에서 OVS 실행
    - OVN 데이터베이스는 NB/SB로 구성
        - NB : 논리적 네트워크 구성 정보. ovn-northd / neutron 플러그인.
        - SB : 논리적 데이터 경로 흐름. ovn-nortd / ovn-controller(각 하이퍼바이저에서 동작). 계산된 경로를 저장한다.
    - 물리적 네트워크 테이블 : Overcloud 노드에 연결하는 방법을 지정(하이퍼바이저)
    - 논리적 네트워크 테이블 : 논리적 데이터 경로 흐름을 지정(ovn-northd)
    - 바인딩 테이블 : 논리 네트워크 구성 요소의 위치를 물리 네트워크에 연결(하이퍼바이저)

- OVN 및 OpenFlow
    - OVN은 OpenFlow프로토콜을 사용하여 관리.
    - OpenFlow는 흐름 테이블을 동적으로 재작성 가능(기능 추가/제거)

- OVN 논리 플로우
    - 플로우를 수동으로 프로그래밍하는것은 관리하기 거의 불가능
    - OVN의 SDN 컨트롤러는 모든 네트워크 구성 요소에서 자동으로 플로우 생성
    - 논리 플로우는 각 하이퍼바이저의 ovn-controller에 배포
    - ovn-controller는 논리 플로우를 OpenFlow로 변환.

- P103 내가그린 장표 참조. 하나의 컴퓨트 노드 및 VM이 어떻게 내부에서 브릿지를 통해 연결되는지 그림.
    - br-int : 내부 스위치
    - br-ex : 외부로 통하는 인터넷 망 관리.
    - br-tun : 내부로 네트워크로 통하는 망 관리


### 테넌트, 프로바이더 및 외부 네트워크 구현

- 도메인 운영자는 테넌트, 라우터, 프로바이더, 외부네트워크 중에서 선택
- SDN은 소프트웨어 레이어에서 네트워크와 서브넷을 정의하고 관리

- 프로젝트 네트워킹
    - P105 장표 참조.
    - 셀프 서비스 네트워크 또는 테넌트 네트워크라 부름
    - 오픈스택 설치 시 일반적인 네트워크 유형
    - 프로젝트 내의 인스턴스만 프로젝트 네트워크를 사용. 외부적인 연결은 안됨. 외부적으로 연결하려면 별도로 소프트웨어적 네트워크를 만들어서 해 주어야 함.
    - 물리적 네트워크와 관련 없음(격리)
    - 라우터를 통해 다른 프로젝트 네트워크나 외부 네트워크에 연결. 장표를 보면 Network A2가 공유로 되어있으나 라우터를 거치지 않는다면 각각 다른 프로젝트의 인스턴스끼리 다이렉트 통신은 불가하다.
    - 외부에서 인스턴스에 직접 접근하려면 floating IP사용.

- 프로바이더 네트워크
    - 데이터 센터 네트워크라는 용어와 동일.
    - 데이터센터의 물리네트워크 VLAN에 생성.
    - 레드햇 오픈스택의 기본 오버레이 프로토컬은 GENEVE
    - 물리적 네트워크 인프라에서 직접 가져온 주소를 사용. 프로젝트 네트워크는 사설주소를 쓰나 여기는 실제 네트워크 주소를 사용한다. 이 타입은 인스턴스들이 각각 공인주소를 갖는다.
    - 물리적 네트워크 라우터를 기본 게이트웨이로 사용
    - DHCP 활성화 시 데이터센터 DHCP서버와 충돌 발생. 프로젝트 네트워킹은 내부주소를 쓰니까 괜찮으나 여기는 외부 주소를 그대로 가져다 쓰니까 안된다.
    - 관리자만 생성 가능(각 프로젝트와 공유)
    - 프로바이더 네트워크 유형은 flat(네트워크 당 하나의 NIC) 또는 VLAN(단일 NIC)

- commands
    - `openstack network show` : 관리할때 사용하는 네트워크 속성 정보 표시.
    - `openstack subnet create` : 특성을 가진 네트워크를 구성할 때 IP주소를 어떻게 쓸거냐 관리.
        - `--network` : 어느 네트워크에 대한 서브넷 설정인가?
        - `--dhcp` : DHCP 쓸거냐?
        - `--gateway` : 게이트웨이는 어떻게 되나
    - `ovs-vsctl show` : OVS 브릿지 인터페이스 정보 확인. br-ex, br-int, br-tun 등의 정보를 볼 수 있다.
    - `openstack hypervisor list` : 컴퓨트 노드들의 정보를 확인할 수 있다. 당연히 어드민만 사용할 수 있다.
...
## Chapter 8.  사용자 지정된 클라우드 애플리케이션 시작 자동화

### Cloud-init을 통한 시작시 인스턴스 사용자 지정

- Cloud-init
    - cloud-init은 오픈스택의 하위서비스는 아님.
    - 인스턴스의 초기화를 처리하는 소프트웨어
        - server create 등으로 이미지를 실행할 때, 어떻게 구성할 지 양식에 맞게 파일을 작성.
    - rhel-guest-image RPM 패키지에 포함되어 있음.
    - host name, ssh key, 임시 마운트 지점 등의 구성 설정 자동화
    - user-data로 호출해 원하는 구성 가능.

- 사용자 지정 스크립트
    - 필요한 구성을 스크립트로 구성해 rc.local 수준의 부팅 프로세스에서 작업.

- 인스턴스 사용자 지정
    - cloud-config 구문으로도 구성 가능
    - yum으로 시스템 업데이트 및 설치 가능
    - ssh 키 복사 및 사용자 생성 가능
    - yaml 형식의 파일로 구성.

### 클라우드 애플리케이션 아키텍처 설명

- 클라우드 애플리케이션은 여러 구성요소로 구성
- 스택(리소스들의 집합. 동적으로 관리할 때 사용하는 단위. 템플릿 단위로 한번에 만들어 지는 단위.)으로 리소스 배포 시 수동 배포보다 효율적

- 클라우드 기반 공통 스택의 구조
    - 클라이언트 - 서버 시스템에서 사용하는 3계층 구성
        - 프리젠테이션 계층 : 프론트엔드, 사용자 인터페이스 제공, api호출로 다른 계층과 통신
        - 애플리케이션 계층 : 콘텐츠와 비즈니스 로직 처리
        - 데이터 계층 : 데이터 스토리지 시스템에 정보 저장.

- 부하 분산
    - LBaas(LB as a Service)는 입력되는 요청을 인스턴스 사이에 균등하게 분배.
    - 분배 방식 : round robin, source ip, least connection

- 이미지 관리
    - 사전 구성 시 시간 절약 및 일관성 보장(버전 및 라이선스 문제 발생 가능성)
    - 기본 이미지 사용 시 유연성은 높지만 생성시간 증가 및 불일치 초래

- 영구 스토리지
    - cinder로 인스턴스에 볼륨 연결
    - swift로 전체 데이터 세트의 내구성 및 가용성 보장.

- 네트워크 서브넷
    - 계층 별 서브넷 생성 및 라우터를 통한 서브넷 간의 트래픽 라우팅
    
- 네트워크 보안
    - 보안 그룹으로 포트/ip등으로 인바운드 규칙 설정
...
- 원격 분석
    - 사용량 분석
    - 모니터링을 통해 임계값에 따른 스케일 업/다운 정책 관리.

- 애플리케이션 자격증명
    - 소유자에게 단일 키스톤 프로젝트에 관한 액세스 권한을 부여하는 인증 메커니즘
    - 기존에는 고유한 사용자 계정 및 전용 사용자로 작업
    - 권한이 많고 암호가 하드 코딩되어 보안에 취약
    - 암호 변경시 모든 애플리케이션 수정 필요
    - 사용자 계정의 하위 집합으로 애플리케이션 자격증명 구축(암호 대신 시크릿 사용)

### 클라우드 애플리케이션 스택 시작

- 오케스트레이션 서비스
    - 클라우드 리소스의 프로비저닝을 자동화
    - Heat는 오픈스택에 오케스트레이션 기능 제공
    - 클라우드에 생성할 리소스 지정
    - 하나 이상의 템플릿으로 스택 구성
    - HOT파일로 스택 배포시 restful API 사용
    - 여러 스택 계층을 순서에 따라 빌드하거나 중첩 스택을 생성할 수도 있음.
    - Heat 엔진에서는 스택의 템플릿을 배포하기 전에 유효성 검증 및 순환 종속성 확인
    - `--dry-run`옵션으로 생성 전 확인 가능
    - 알람/임계값/신호/원격분석 서비스 등으로 리소스 자동 확장 가능.



## Chapter 9. 클라우드 애플리케이션 배치 관리

### 다중 사이트 Overcloud 배포 설명

- 다중 사이트에 맞는 오버클라우드 설계 고려 사항
    - 지리적으로 분산해서 여러 데이터센터 운영
        - 네트워크 성능에 따라 속도 지연 발생 가능성.
    - 네트워크 대기 시간과 중앙 집중식 관리의 이점을 고려해 다중 사이트 설계

- DCN(Distributed Compute Node) 아키텍처
    - 컴퓨트 노드만 원격 사이트에 배포하는 방식
    - 각 역할을 이용해 일반 컴퓨트 노드 및 HCI 노드 배포
    - 기본 배포시 GlanceAPIEdge 및 추가 배포시 HAProxyEdge 서비스 포함
    - 중앙 사이트와 원격 사이트의 왕복 시간이 100ms 이내여야 함
    - Overcloud 배포 시 DCN 아키텍처는 여러개의 Heat Stack 사용 지원.

- 여러 사이트에서 단일 오버스클라우드 스택 사용
    - 단일 스택으로 다중 사이트 구성 가능
    - 사이트 간의 네트워크 연결 이중화
    - 네트워크 대기 시간 100ms 이내로 구성
    - 네트워크 중단 시 배포에 심각한 문제 발생
    - 계획 단계부터 잠재적 오류에 대비
    - 컴퓨트 노드의 오류 대비

- 분리 구조
    - 리소스 분리는 대규모 수평 확장에 중요한 요소
    - 리소스 분리 : 물리적 특성/기능을 기반으로 인프라 그룹화
    - 원크로드 분리는 호스트 수준 또는 CPU수준에서 작동

- Region
    - Overcloud 배포 내에서 구분하는 단위
    - 모든 사이트를 지원하는 단일 ID서비스가 있는 다중 사이트 아키텍처
    - 영역 관리는 admin 사용자만 가능(어드민역할이 아님)
    - 레드햇에서는 권장하지 않는 방식
